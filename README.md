...
**Current decoder replay/runtime coverage** (`263/263` decoder tests passing):
...
- Added a narrow AVR-side `36-2-2-2` state suite that locks the current decoder's maintained `4`-cylinder sync-entry paths (`tooth 19` via consecutive long gaps and `tooth 35` via long-gap then normal tooth) plus synced wrap behavior without claiming real-capture tooth numbering or limiting the pattern to only one engine family
...
- Formalized the existing `36-2-2-2` AVR-side state slice around the already-verified maintained `4`-cylinder tooth-`19` and tooth-`35` sync-entry paths plus synced wrap behavior, while keeping the older evidence/staging note as the capture source
...
- Added a narrow AVR-side `DRZ400` suite that locks setup-seeded filters, unsynced secondary sync onto tooth `6`, synced secondary realignment, and first-primary wrap through the shared dual-wheel ISR without changing production decoder logic
...
- [SESSION_HANDOFF_2026-03-23_DRZ400_STATE.md](speeduino/SESSION_HANDOFF_2026-03-23_DRZ400_STATE.md)
