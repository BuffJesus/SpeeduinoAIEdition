...
static void setup_trace_subaru67(void)
{
    reset_trace_runtime();
    configPage4.TrigSpeed = CRANK_SPEED;
    configPage4.sparkMode = IGN_MODE_SEQUENTIAL;
...
static void setup_trace_rover_mems_primary_only(void)
{
    reset_trace_runtime();
    configPage4.TrigPattern = DECODER_ROVERMEMS;
    configPage4.TrigSpeed = CRANK_SPEED;
    configPage4.trigPatternSec = SEC_TRIGGER_5_3_2;
    configPage4.sparkMode = IGN_MODE_WASTED;
    configPage4.triggerAngle = 0;
    configPage4.triggerFilter = TRIGGER_FILTER_LITE;
    configPage2.nCylinders = 4U;
...
static void build_rover_wheel_bits(const uint8_t *runs, size_t runCount, char *wheelBits)
{
    size_t cursor = 0U;
    for (size_t index = 0; index < runCount; ++index)
    {
        for (uint8_t count = 0U; count < runs[index]; ++count)
        {
            wheelBits[cursor++] = '1';
        }
        wheelBits[cursor++] = '0';
...
static void replay_rover_primary_runs_until_pattern(
    const uint8_t *runs,
    size_t runCount,
    uint8_t startSlot,
...
{
    char wheelBits[36];
    build_rover_wheel_bits(runs, runCount, wheelBits);

    uint32_t deltaUs = 0U;
    for (size_t step = 0; step < 108U; ++step)
    {
        const size_t slot = (startSlot + step) % 36U;
        deltaUs += 1000U;
        if (wheelBits[slot] != '1')
        {
            continue;
        ...
        if (toothAngles[ID_TOOTH_PATTERN] == expectedPattern)
        {
            return;
        }
    }
...
static TraceReplayCallbacks makeSubaru67Callbacks(void)
{
    const TraceReplayCallbacks callbacks = {triggerPri_Subaru67, nullptr, triggerSec_Subaru67, nullptr};
    return callbacks;
}
...
static void test_trace_replay_subaru67_three_cam_pulses_establish_sync_on_tooth_two(void)
{
    setup_trace_subaru67();

    replayTriggerTrace(makeTriggerTrace(kSubaru67SyncEvents), makeSubaru67Callbacks());
...
static void test_trace_replay_subaru67_synced_cycle_wraps_after_twelve_primary_teeth(void)
{
    setup_trace_subaru67();

    replayRepeatedTriggerTrace(makeRepeatedTriggerTrace(kSubaru67WrapEvents), makeSubaru67Callbacks());
...
static void test_trace_replay_subaru67_single_cam_pulse_at_wrong_tooth_realigns_tooth_five_without_sync(void)
{
    setup_trace_subaru67();

    replayTriggerTrace(makeTriggerTrace(kSubaru67SingleCamMisalignedEvents), makeSubaru67Callbacks());
...
static void test_trace_replay_rover_mems_mems3_primary_only_identifies_pattern_three(void)
{
    const uint8_t runs[] = {2U, 14U, 3U, 13U};

    setup_trace_rover_mems_primary_only();
    replay_rover_primary_runs_until_pattern(runs, sizeof(runs), 27U, 3U);

    TEST_ASSERT_EQUAL_UINT8(3U, toothAngles[ID_TOOTH_PATTERN]);
    TEST_ASSERT_FALSE(currentStatus.hasSync);
    TEST_ASSERT_TRUE(BIT_CHECK(currentStatus.status3, BIT_STATUS3_HALFSYNC));
    TEST_ASSERT_EQUAL_UINT16(1U, toothCurrentCount);
    TEST_ASSERT_EQUAL_UINT8(8U, toothAngles[SKIP_TOOTH1]);
    TEST_ASSERT_EQUAL_UINT8(10U, toothAngles[SKIP_TOOTH2]);
    TEST_ASSERT_EQUAL_UINT8(24U, toothAngles[SKIP_TOOTH3]);
    TEST_ASSERT_EQUAL_UINT8(27U, toothAngles[SKIP_TOOTH4]);
    TEST_ASSERT_EQUAL_UINT8(0U, currentStatus.syncLossCounter);
}

static void test_trace_replay_rover_mems_mems19_primary_only_identifies_pattern_two(void)
{
    const uint8_t runs[] = {11U, 5U, 12U, 4U};

    setup_trace_rover_mems_primary_only();
    replay_rover_primary_runs_until_pattern(runs, sizeof(runs), 34U, 2U);

    TEST_ASSERT_EQUAL_UINT8(2U, toothAngles[ID_TOOTH_PATTERN]);
    TEST_ASSERT_FALSE(currentStatus.hasSync);
    TEST_ASSERT_TRUE(BIT_CHECK(currentStatus.status3, BIT_STATUS3_HALFSYNC));
    TEST_ASSERT_EQUAL_UINT16(1U, toothCurrentCount);
    TEST_ASSERT_EQUAL_UINT8(1U, toothAngles[SKIP_TOOTH1]);
    TEST_ASSERT_EQUAL_UINT8(12U, toothAngles[SKIP_TOOTH2]);
...
void testTriggerTraceReplay(void)
{
    SET_UNITY_FILENAME() {
        ...
        RUN_TEST_P(test_trace_replay_subaru67_three_cam_pulses_establish_sync_on_tooth_two);
        RUN_TEST_P(test_trace_replay_subaru67_synced_cycle_wraps_after_twelve_primary_teeth);
