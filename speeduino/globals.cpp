...
uint8_t getBoardCapabilityFlags(uint8_t pinMapping)
{
  ...
  flags |= getBoardSpecificCapabilityFlags(pinMapping);

  return flags;
}

bool boardHasCapability(uint8_t capabilityFlag)
{
  return (getBoardCapabilityFlags(configPage2.pinMapping) & capabilityFlag) != 0U;
}
