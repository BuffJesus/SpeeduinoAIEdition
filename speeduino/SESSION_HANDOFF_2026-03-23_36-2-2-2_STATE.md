...
Focus: Formalize the current `36-2-2-2` maintained `4`-cylinder sync-path contract as part of the Phase 2 decoder baseline without implying the pattern is exclusive to only one engine family
...
- the maintained slice is about the current in-tree `4`-cylinder ISR behavior, not about claiming real-capture tooth numbering beyond what the direct state fixture safely encodes or about limiting real-world `36-2-2-2` use to only Subaru H4 applications
...
- Kept the earlier Subaru-focused evidence note in [SESSION_HANDOFF_2026-03-22_36-2-2-2.md](C:/Users/Cornelio/Desktop/speeduino-202501.6/speeduino/SESSION_HANDOFF_2026-03-22_36-2-2-2.md) as the source for why this narrower state-model slice is the safe scope
...
- synced wrap and revolution counting on the maintained `4`-cylinder path
...
- passing with the `36-2-2-2` maintained `4`-cylinder state model formalized in the full decoder entrypoint
