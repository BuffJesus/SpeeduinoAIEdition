...
#ifdef UNIT_TEST
static updates_test_state updatesTestState = {};
...
{
  updatesTestState.useFakeVersion = true;
  updatesTestState.eepromVersion = version;
  updatesTestState.lastStoredVersion = version;
}

void updatesTestSetInterceptStorageCalls(bool interceptStorageCalls)
{
  updatesTestState.interceptStorageCalls = interceptStorageCalls;
}

void updatesTestSetStopAfterStore(bool stopAfterStore)
{
  updatesTestState.stopAfterStore = stopAfterStore;
}

updates_test_state updatesTestGetState(void)
{
  return updatesTestState;
}
...
static uint8_t updatesReadEEPROMVersion()
{
  if(updatesTestState.useFakeVersion)
  {
    ++updatesTestState.readVersionCalls;
    if(updatesTestState.stopAfterStore && (updatesTestState.storeVersionCalls > 0U))
    {
      return CURRENT_DATA_VERSION;
    }
    return updatesTestState.eepromVersion;
  }

  return readEEPROMVersion();
}

static void updatesStoreEEPROMVersion(byte newVersion)
{
  if(updatesTestState.useFakeVersion)
  {
    updatesTestState.eepromVersion = newVersion;
    updatesTestState.lastStoredVersion = newVersion;
    ++updatesTestState.storeVersionCalls;
    return;
  }

  storeEEPROMVersion(newVersion);
}

static void updatesWriteAllConfig()
{
  if(updatesTestState.interceptStorageCalls)
  {
    ++updatesTestState.writeAllConfigCalls;
    return;
  }
...
static void updatesLoadConfig()
{
  if(updatesTestState.interceptStorageCalls)
  {
    ++updatesTestState.loadConfigCalls;
    return;
  }
...
static void updatesWriteCalibration()
{
  if(updatesTestState.interceptStorageCalls)
  {
    ++updatesTestState.writeCalibrationCalls;
    return;
  }
...
#define readEEPROMVersion updatesReadEEPROMVersion
#define storeEEPROMVersion updatesStoreEEPROMVersion
#define writeAllConfig updatesWriteAllConfig
#define loadConfig updatesLoadConfig
#define writeCalibration updatesWriteCalibration
#endif
...
#ifdef UNIT_TEST
// Testable migration helper for v13→v14 PID gain rescaling
// Old resolution: 100% per increment (0=0%, 1=100%, 2=200%, etc.)
// New resolution: 100% = 32 (0=0%, 32=100%, 64=200%, etc.)
// Values ≥8 would overflow (8*32=256), so clamp to UINT8_MAX
uint8_t migratePIDGain_v13_to_v14(uint8_t oldGain) {
  if (oldGain >= 8) {
    return UINT8_MAX;
  }
  return oldGain << 5; // Multiply by 32
}

// Testable migration helper for v13→v14 cranking enrichment rescaling
// Old resolution: 0-255% (1:1 percentage), New resolution: 0-1275% (5% per unit)
// Converts by dividing by 5 to fit higher percentage range into same byte
uint8_t migrateCrankingEnrich_v13_to_v14(uint8_t oldValue) {
  return oldValue / 5;
}

// Testable migration helper for v13→v14 flex fuel advance offset adjustment
// Old: Stored directly in degrees, New: Stored with +40 degree offset (same as main ignition table)
// Add 40 to convert old value to new format
uint8_t migrateFlexAdvance_v13_to_v14(uint8_t oldAdvance) {
  // Check for overflow: if adding 40 would exceed UINT8_MAX (255), clamp to UINT8_MAX
  if (oldAdvance > 215) {
    return UINT8_MAX;
  }
  return oldAdvance + 40;
}

// Testable migration helper for v4→v5 cranking enrichment curve expansion
// Old: Single percentage value, New: 4-point curve with each value = 100 + old percentage
// Converts old enrichment percentage to new curve value format (baseline 100% + enrichment)
uint8_t migrateCrankingPctToCurve_v4_to_v5(uint8_t oldCrankingPct) {
  // Check for overflow: if adding 100 would exceed UINT8_MAX (255), clamp to UINT8_MAX
  if (oldCrankingPct > 155) {
    return UINT8_MAX;
  }
  return 100 + oldCrankingPct;
}

// Testable migration helper for v18→v19 TPS resolution upgrade
// Old: 1.0% resolution (0-100% mapped to 0-100), New: 0.5% resolution (0-100% mapped to 0-200)
// Multiply by 2 to convert old TPS values to new resolution
uint8_t migrateTPSResolution_v18_to_v19(uint8_t oldTPSValue) {
  // Check for overflow: if multiplying by 2 would exceed UINT8_MAX (255), clamp to UINT8_MAX
  if (oldTPSValue > 127) {
    return UINT8_MAX;
  }
