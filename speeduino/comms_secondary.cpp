...
void secondserial_Command(void)
{
  ...
  if(shouldRouteSecondaryTunerStudioToPrimary(configPage9.secondarySerialProtocol, currentStatus.status5))
  {
  ...
  switch (currentSecondaryCommand)
  {
    ...
    case 'n':
    {
      ...
      const SecondaryRealtimeRequest request = buildSecondaryRealtimeRequest(currentSecondaryCommand, configPage9.secondarySerialProtocol);
      if (request.supported)
      {
        if(request.useLegacyLayout) { sendValues(0, request.packetLength, request.cmd, secondarySerial, serialSecondaryStatusFlag, &getLegacySecondarySerialLogEntry); }
        else { sendValues(0, request.packetLength, request.cmd, secondarySerial, serialSecondaryStatusFlag); }
      }
      break;
    }
    ...
    case 'S': // send code version
      legacySerialHandler(getLegacyVersionResponseCommand(currentSecondaryCommand, true, configPage9.secondarySerialProtocol), secondarySerial, serialSecondaryStatusFlag);
