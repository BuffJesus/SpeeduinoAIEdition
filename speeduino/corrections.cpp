...
void initialiseCorrections(void)
{
  ...
  PID_O2 = 0L;
  PID_AFRTarget = 0L;
  egoPID.Initialize();     // Reset PID internal state (ITerm, lastInput) to current zeroed output/input values
  egoPID.SetMode(AUTOMATIC); // Ensure PID runs in automatic mode
