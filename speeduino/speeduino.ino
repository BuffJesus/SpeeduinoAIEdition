...
{
  uint16_t maxAllowedRPM = checkRevLimit(); // Returned in RPM/100 for rev-limit and protection tables.
  if(checkEngineProtect() && (configPage4.engineProtectMaxRPM < maxAllowedRPM)) { maxAllowedRPM = configPage4.engineProtectMaxRPM; }
  if(currentStatus.launchingHard && (configPage6.lnchHardLim < maxAllowedRPM)) { maxAllowedRPM = configPage6.lnchHardLim; }

  maxAllowedRPM = maxAllowedRPM * 100U;
  if(currentStatus.flatShiftingHard && (currentStatus.clutchEngagedRPM < maxAllowedRPM)) { maxAllowedRPM = currentStatus.clutchEngagedRPM; }
...
table3d_value_t getVE1Runtime(void)
{
  ...
  tempVE = getExperimentalNativeU16Page2Value(currentStatus.fuelLoad, currentStatus.RPM); //Perform lookup into fuel map for RPM vs MAP value
...
void checkLaunchAndFlatShift()
{
  ...
  BIT_CLEAR(currentStatus.status5, BIT_STATUS5_FLATSH);
