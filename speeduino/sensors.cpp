...
TESTABLE_INLINE_STATIC bool knockIsSuppressed(const statuses &current, const config10 &page10)
{
  if(page10.knock_limiterDisable == 0U) { return false; }

  return current.launchingHard
      || current.launchingSoft
      || current.flatShiftingHard
      || BIT_CHECK(current.status2, BIT_STATUS2_HLAUNCH)
      || BIT_CHECK(current.status2, BIT_STATUS2_SLAUNCH)
      || BIT_CHECK(current.status5, BIT_STATUS5_FLATSH)
      || BIT_CHECK(current.status5, BIT_STATUS5_FLATSS);
}

TESTABLE_INLINE_STATIC bool knockAngleIsInsideWindow(int currentAngle, int windowStart, int windowDuration, int angleMax)
{
  if((currentAngle < 0) || (windowDuration <= 0) || (angleMax <= 0)) { return false; }

  while(windowStart < 0) { windowStart += angleMax; }
  while(windowStart >= angleMax) { windowStart -= angleMax; }

  int windowEnd = windowStart + windowDuration;
  if(windowEnd < angleMax) { return (currentAngle >= windowStart) && (currentAngle < windowEnd); }

  windowEnd -= angleMax;
  return (currentAngle >= windowStart) || (currentAngle < windowEnd);
}

TESTABLE_INLINE_STATIC bool knockWindowIsActive(const statuses &current, const config10 &, int currentAngle)
{
  if( !(current.hasSync || BIT_CHECK(current.status3, BIT_STATUS3_HALFSYNC)) ) { return false; }

  int16_t windowStart = table2D_getValue(&knockWindowStartTable, current.RPMdiv100);
  int16_t windowDuration = table2D_getValue(&knockWindowDurationTable, current.RPMdiv100);
  return knockAngleIsInsideWindow(currentAngle, windowStart, windowDuration, CRANK_ANGLE_MAX);
}
...
//
static inline int readBoardAnalogPinNormalized(uint8_t pin)
{
#if defined(CORE_TEENSY41)
  if (boardHasCapability(BOARD_CAP_12BIT_ADC))
  {
    return analogRead10bit(pin);
  }
#endif
...
static inline uint16_t readAnalogPin(uint8_t pin)
{
  ...
  analogRead(pin);
  // Downstream sensor math expects 10-bit ADC values.
  // Boards with higher-resolution ADCs normalize here before any sensor conversion.
  int tmp = readBoardAnalogPinNormalized(pin);
...
*/
TESTABLE_INLINE_STATIC bool isValidBaro(uint8_t baro)
{
...
bool knockEventIsValid(void)
{
  if(knockIsSuppressed(currentStatus, configPage10)) { return false; }
  if(!knockIsInsideLimits(currentStatus, configPage10)) { return false; }

  return knockWindowIsActive(currentStatus, configPage10, getCrankAngle());
}
