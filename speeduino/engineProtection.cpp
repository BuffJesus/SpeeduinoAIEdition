...
static bool oilProtectCountEnabled = false;
static bool afrProtectActive = false;
static bool afrProtectCountEnabled = false;
static unsigned long afrProtectCount = 0;

static inline void resetOilProtectionTimer(void)
{
  oilProtStartTime = 0;
  oilProtectCountEnabled = false;
}

static inline void resetAFRProtectionState(void)
{
  afrProtectActive = false;
  afrProtectCountEnabled = false;
  afrProtectCount = 0;
  BIT_CLEAR(currentStatus.engineProtectStatus, ENGINE_PROTECT_BIT_AFR);
}

static inline bool afrProtectionConfigured(void)
{
  return (configPage6.engineProtectType != PROTECT_CUT_OFF) && (configPage9.afrProtectEnabled > 0) && (configPage6.egoType == EGO_TYPE_WIDE);
}

bool protectionTimerElapsed(uint8_t currentTime, uint8_t startTime, uint8_t delayTime)
{
  return (uint8_t)(currentTime - startTime) >= delayTime;
}
    ...
    if( (configPage6.boostCutEnabled > 0) && (currentStatus.MAP > (configPage6.boostLimit * 2)) ) //The boost limit is divided by 2 to allow a limit up to 511kPa
    {
      ...
      switch(configPage6.engineProtectType)
      {
        ...
        case PROTECT_CUT_BOTH:
          ...
          break;
        default:
          //Shouldn't happen, but avoid stale cut indicators if protect type is invalid.
          BIT_CLEAR(currentStatus.status1, BIT_STATUS1_BOOSTCUT);
          BIT_CLEAR(currentStatus.status2, BIT_STATUS2_BOOSTCUT);
          boostLimitActive = 0;
          BIT_CLEAR(currentStatus.engineProtectStatus, ENGINE_PROTECT_BIT_MAP);
...
byte checkOilPressureLimit(void)
{
  ...
  uint8_t currentTime = uint8_t(div100(millis()));
  ...
  if (configPage6.engineProtectType != PROTECT_CUT_OFF) 
  {
    if( (configPage10.oilPressureProtEnbl == true) && (configPage10.oilPressureEnable == true) )
    {
      ...
      if(currentStatus.oilPressure < oilLimit)
      {
        ...
        if(!oilProtectCountEnabled)
        {
          oilProtStartTime = currentTime;
          oilProtectCountEnabled = true;
        }

        /* Check if countdown has reached its target, if so then instruct to cut */
        if( protectionTimerElapsed(currentTime, oilProtStartTime, configPage10.oilPressureProtTime) || (alreadyActive > 0) )
        {
      ...
      else { resetOilProtectionTimer(); } //Reset the timer
    }
  }
  else
  {
    resetOilProtectionTimer();
  }
  ...
  */
  if(afrProtectionConfigured()) {
    ...
    */
    ...
    if(afrProtectActive && (currentStatus.TPS <= configPage9.afrProtectReactivationTPS)) 
    {
      resetAFRProtectionState();
    }
  }
  else
  {
